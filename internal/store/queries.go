...
// expected.
func (q *Queries) SearchFields(ctx context.Context, version, query string, limit int) ([]SearchResult, error) {
	if limit <= 0 {
		limit = 25
	}
	...
	sanitized = splitCamelCase(sanitized)
	sanitized = implicitOR(sanitized)
